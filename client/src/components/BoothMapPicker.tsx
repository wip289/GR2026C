            ...
            return (
              <g key={booth.id}
                ...
                <rect x={booth.x} y={booth.y} width={booth.w} height={booth.h}
                  rx="2" fill={fill} stroke={stroke} strokeWidth={sw} opacity={0.93}/>
                {isSel && (
                  <text x={booth.x+booth.w/2} y={booth.y+12}
                    textAnchor="middle" fill="#6ee7b7" fontSize="9" fontWeight="900">✓</text>
                )}
                <text x={booth.x+booth.w/2} y={booth.y+booth.h/2+(isSel?4:0)}
                  textAnchor="middle" dominantBaseline="central"
                  fill={COLORS[booth.status].text} fontSize={fs} fontWeight="700">
                  {booth.label}
                </text>
              </g>
            );
