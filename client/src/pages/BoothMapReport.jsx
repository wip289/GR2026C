...
// Booth yang disembunyikan sementara (data tetap ada, tidak ditampilkan)
const HIDDEN_BOOTHS = new Set(["S26","S27","S28","S29","S32","S33","S34","S35","M9","M12"]);

const INIT_ASSIGNMENTS = {
  M2:  { company:"PT. Indosamudera Service Management", status:"booked" },
  M8:  { company:"PT. Archipelago Intern",              status:"booked" },
  M9:  { company:"Booth Sponsor",                       status:"sponsor" },
  M10: { company:"Booth Sponsor",                       status:"sponsor" },
  M11: { company:"Booth Sponsor",                       status:"sponsor" },
  S3:  { company:"PT. AYANA Hotel Manajemen",           status:"booked" },
  S4:  { company:"Club Med Phuket",                     status:"booked" },
  S25: { company:"Booth Sponsor",                       status:"sponsor" },
  S30: { company:"Booth Sponsor",                       status:"sponsor" },
  S31: { company:"Booth Sponsor",                       status:"sponsor" },
  S36: { company:"Booth Sponsor",                       status:"sponsor" },
};

const TABLE_SECTIONS = [
  {
    label: "MAIN BOOTH — 5 × 5 m",
    rows: Array.from({length:12},(_,i)=>({ no:i+1, id:`M${i+1}` })),
  },
  {
    label: "STANDARD BOOTH — 3 × 3 m",
    rows: Array.from({length:36},(_,i)=>({ no:i+1, id:`S${i+1}` })),
  },
  {
    label: "EXTRA BOOTH — 4 × 2 m",
    rows: [{no:1,id:"E1"},{no:2,id:"E2"},{no:3,id:"E3"},{no:4,id:"E4"}],
  },
];

function boothFill(id, type, asgn) {
  if (type==="area")  return { fill:"#c9a84c", stroke:"#9a7b2e", text:"#fff" };
  if (type==="staff") return { fill:"#e2e8f0", stroke:"#94a3b8", text:"#64748b" };
  const a = asgn[id];
  if (!a) {
    if (type==="main")  return { fill:"#f0f4f8", stroke:"#94a3b8", text:"#334155" };
    if (type==="extra") return { fill:"#f0f4f8", stroke:"#94a3b8", text:"#334155" };
    return { fill:"#f8fafc", stroke:"#cbd5e1", text:"#475569" };
  }
  if (a.status==="booked")  return { fill:"#4ade80", stroke:"#16a34a", text:"#fff" };
  if (a.status==="sponsor") return { fill:"#93c5fd", stroke:"#3b82f6", text:"#fff" };
  return { fill:"#f8fafc", stroke:"#cbd5e1", text:"#475569" };
}

function BoothLabel({ b, asgn }) {
  const cx = b.x + b.w/2, cy = b.y + b.h/2;
  const c = boothFill(b.id, b.type, asgn);
  const a = asgn[b.id];

  if (b.type==="area") {
    return <text x={cx} y={cy+4} textAnchor="middle" fontSize={b.h>50?14:10} fill="#fff" fontWeight="bold">{b.label}</text>;
  }
  if (b.type==="staff") {
    return <text x={cx} y={cy+4} textAnchor="middle" fontSize="8" fill="#64748b">{b.label}</text>;
  }

  const isLarge = b.type==="main" || b.type==="extra";
  if (!a) {
    return <text x={cx} y={cy+4} textAnchor="middle" fontSize={isLarge?12:9} fill={c.text} fontWeight="600">{b.label}</text>;
  }

  // Assigned booth: ID kecil di atas, nama perusahaan 2 baris
...
export default function BoothMapReport() {
  const [asgn] = useState(INIT_ASSIGNMENTS);
    ...
    <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",padding:"12px",background:"#f8fafc",minHeight:"100vh"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:"10px",paddingBottom:"8px",borderBottom:"3px solid #1d4ed8"}}>
        <div>
          ...
          <div style={{fontSize:"11px",color:"#64748b",marginTop:"1px"}}>Gedung Dome, NHI Bandung &nbsp;·&nbsp; 10–11 Juni 2026 &nbsp;·&nbsp; Data per {today}</div>
        </div>
      ...
      <div style={{display:"flex",gap:"12px",alignItems:"flex-start"}}>
        ...
        <div style={{flex:"0 0 52%",background:"#fff",border:"1px solid #e2e8f0",borderRadius:"8px",padding:"8px"}}>
          <svg viewBox={`0 0 ${VW} ${VH}`} style={{width:"100%",height:"auto",display:"block"}}>
            ...
            {/* Central aisle line (standard section) */}
            ...
            {/* Central aisle line (main section) */}
            ...
            {ALL_BOOTHS.filter(b=>!HIDDEN_BOOTHS.has(b.id)).map(b=>{
              const c = boothFill(b.id, b.type, asgn);
              const sw = b.type==="main"||b.type==="extra" ? 1.5 : 1;
              return (
                <g key={b.id}>
                  <rect x={b.x} y={b.y} width={b.w} height={b.h} fill={c.fill} stroke={c.stroke} strokeWidth={sw} rx="3"/>
