...
const STEPS = ["Data Diri", "Latar Belakang", "Dokumen", "Consent", "Selesai"];
...
const STATUS_OPTIONS = [
  { value: "mahasiswa", label: "Mahasiswa Aktif", desc: "Sedang menempuh pendidikan D3/D4/S1" },
  { value: "fresh_graduate", label: "Fresh Graduate", desc: "Lulus dalam 2 tahun terakhir" },
  { value: "alumni_nhi", label: "Alumni NHI Bandung", desc: "Lulusan Politeknik Pariwisata NHI Bandung" },
  { value: "umum", label: "Pencari Kerja Umum", desc: "Berpengalaman atau pindah karir" },
];
...
const s = {
  ...
  wrap:    { maxWidth: 720, margin: "0 auto", padding: "2rem 1.25rem" },
...
async function uploadFile(file: File, type: "foto" | "cv" | "ktm" | "sertifikat", registrationId: string): Promise<string | null> {
  try {
    ...
    const res = await fetch(`/api/upload?type=${type}&registrationId=${registrationId}`, {
      method: "POST",
      body: formData,
    });
...
export default function JobseekerRegister() {
  ...
  const [registrationId] = useState(() => {
    // Generate temp ID, will be replaced with proper one on submit
    return "JS-TEMP-" + Date.now().toString().slice(-4);
  });
  ...
  const createJobseekerMutation = trpc.event.createJobseeker.useMutation({
    ...
    onError: (err) => {
      console.warn("[JobseekerRegister] DB save failed:", err.message);
      toast.error("Gagal mendaftar", { description: err.message || "Coba lagi atau hubungi panitia." });
    },
  });

  // ── Validation errors (no navigate, stay on page) ──
  ...
  const checkField = async (field: "nik" | "email" | "whatsapp", value: string) => {
    ...
    try {
      ...
      if (isDuplicate) {
        const idLabel = form.idType === "sim" ? "No. SIM" : form.idType === "passport" ? "No. Passport" : "NIK";
        const messages: Record<string, string> = {
          nik: `${idLabel} ini sudah terdaftar. Gunakan nomor identitas yang sesuai kartu Anda.`,
          email: "Email ini sudah digunakan. Gunakan email lain.",
          whatsapp: "Nomor WhatsApp ini sudah terdaftar. Gunakan nomor lain.",
        };
  ...
  const handleSubmit = async () => {
    ...
    const properId = generateJobseekerId({
      namaLengkap: form.namaLengkap,
      institusi: form.institusi,
      tahunLulus: form.tahunLulus,
      isAlumniNHI: form.status === "alumni_nhi",
    });
    setFinalId(properId);

    toast.loading("Mengupload dokumen...", { id: "upload" });

    // Upload files in parallel using uploadFile helper
    ...
    createJobseekerMutation.mutate({
      ...
      tahunLulus: form.tahunLulus || undefined,
      bidangMinat: form.bidangMinat || undefined,
      consent1: form.consent1,
      consent2: form.consent2,
      fotoUrl: fotoUrl || undefined,
      cvUrl: cvUrl || undefined,
      ktmUrl: ktmUrl || undefined,
      sertifikatUrl: sertifikatUrl || undefined,
    });
  };
    ...
    return (
      <div style={{ ...s.page, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ textAlign: "center", maxWidth: 500, padding: "2rem" }}>
          ...
          <h1 style={{ fontSize: "2rem", fontWeight: 800, marginBottom: "0.75rem" }}>Pendaftaran Berhasil!</h1>
          ...
          <div style={{ background: "rgba(212,160,23,0.08)", border: "1px solid rgba(212,160,23,0.2)", borderRadius: 12, padding: "1.25rem", marginBottom: "1rem", textAlign: "left" }}>
            ...
            <div style={{ fontSize: "0.82rem", color: "#64748b", lineHeight: 1.7 }}>
              Simpan ID ini untuk keperluan check-in di hari H.<br/>
              Konfirmasi pendaftaran akan dikirim ke <strong style={{ color: "#f1f5f9" }}>{form.email}</strong>
            </div>
          </div>
          ...
          <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, overflow: "hidden", marginBottom: "1rem" }}>
            ...
            <iframe
              srcDoc={generateIdCardHTML({
                registrationId: finalId || registrationId,
                namaLengkap: form.namaLengkap,
                institusi: form.institusi || undefined,
                jurusan: form.jurusan || undefined,
                bidangMinat: form.bidangMinat || undefined,
          ...
          {/* Consent summary */}
          <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "1.25rem", marginBottom: "1.5rem", textAlign: "left" }}>
            ...
            <div style={{ fontSize: "0.85rem", color: "#94a3b8", marginBottom: "0.4rem" }}>
              {form.consent1 ? "✅" : "❌"} Profil dapat dilihat employer offline GR2026
            </div>
            <div style={{ fontSize: "0.85rem", color: "#94a3b8" }}>
              {form.consent2 ? "✅" : "⬜"} Profil dapat dibagikan ke employer online
            </div>
            <p style={{ fontSize: "0.75rem", color: "#334155", marginTop: "0.75rem", lineHeight: 1.5 }}>
              Kamu dapat mengubah consent kapan saja melalui portal jobseeker.
            </p>
          </div>
    ...
    <div style={s.page}>
      ...
      <div style={s.wrap}>
        <div style={{ textAlign: "center", marginBottom: "2.5rem" }}>
          ...
          <p style={{ color: "#64748b", fontSize: "0.9rem" }}>Grand Recruitment 2026 · June 8–9 · Dome NHI Bandung · <strong style={{ color: "#14b8a6" }}>GRATIS</strong></p>
        </div>
          ...
          {STEPS.map((label, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", flexShrink: 0 }}>
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "0.35rem" }}>
                <div style={{ width: 34, height: 34, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: "0.85rem", background: i < step ? "#B8860B" : i === step ? "#D4A017" : "rgba(255,255,255,0.06)", color: i <= step ? "#fff" : "#475569", border: i === step ? "2px solid #fde68a" : "none" }}>
                  {i < step ? "✓" : i + 1}
                </div>
                <span style={{ fontSize: "0.68rem", color: i === step ? "#D4A017" : "#475569", whiteSpace: "nowrap" }}>{label}</span>
              </div>
              {i < STEPS.length - 1 && <div style={{ height: 2, width: "clamp(16px,4vw,44px)", background: i < step ? "#B8860B" : "rgba(255,255,255,0.06)", margin: "0 0.25rem", marginBottom: "1.3rem", flexShrink: 0 }} />}
            </div>
          ))}
        </div>
        ...
        {step === 0 && (
          <div style={s.card}>
            ...
            <div style={{ marginBottom: "1rem" }}>
              ...
              <p style={s.hint}>Isi nama lengkap sesuai yang tertera di KTP/Kartu Mahasiswa</p>
            </div>
            ...
            <div style={{ marginBottom: "1rem" }}>
              ...
              <input
                ...
                placeholder={form.idType === "ktp" ? "16 digit NIK, contoh: 3273012345670001" : form.idType === "sim" ? "Nomor SIM Anda" : "Nomor Passport Anda"}
                maxLength={form.idType === "ktp" ? 16 : 20}/>
              <p style={s.hint}>
                {form.idType === "ktp" ? "Nomor Induk Kependudukan 16 digit dari KTP." :
                 form.idType === "sim" ? "Nomor SIM yang masih berlaku." :
                 "Nomor Passport aktif Anda."}
              </p>
              {checking.nik && <p style={{...s.hint, color: "#14b8a6"}}>⏳ Memeriksa nomor identitas...</p>}
              {fieldErrors.nik && (
                <div style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 8, padding: "0.65rem 1rem", marginTop: "0.4rem" }}>
                  <p style={{ fontSize: "0.8rem", color: "#f87171", margin: 0 }}>⚠️ {fieldErrors.nik}</p>
                </div>
              )}
            </div>
            ...
            <div style={s.row2}>
              <div>
                ...
                <p style={s.hint}>Nomor aktif yang bisa dihubungi. Panitia dan employer akan menghubungi via WhatsApp ini.</p>
                {checking.whatsapp && <p style={{...s.hint, color: "#14b8a6"}}>⏳ Memeriksa nomor...</p>}
                {fieldErrors.whatsapp && (
                  <div style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 8, padding: "0.65rem 1rem", marginTop: "0.4rem" }}>
                    <p style={{ fontSize: "0.8rem", color: "#f87171", margin: 0 }}>⚠️ {fieldErrors.whatsapp}</p>
                  </div>
                )}
              </div>
              <div>
                ...
                <p style={s.hint}>Email aktif untuk konfirmasi pendaftaran dan komunikasi dari employer.</p>
                {checking.email && <p style={{...s.hint, color: "#14b8a6"}}>⏳ Memeriksa email...</p>}
                {fieldErrors.email && (
                  <div style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 8, padding: "0.65rem 1rem", marginTop: "0.4rem" }}>
                    <p style={{ fontSize: "0.8rem", color: "#f87171", margin: 0 }}>⚠️ {fieldErrors.email}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div style={{ marginTop: "1rem" }}>
              ...
              <p style={s.hint}>Kota tempat tinggal saat ini</p>
            </div>
          </div>
        )}
          ...
          <div style={s.card}>
            ...
            <div style={{ marginBottom: "1rem" }}>
              ...
              <input style={s.input} value={form.institusi} onChange={e => upd("institusi", e.target.value)}
                placeholder={form.status === "alumni_nhi" ? "Politeknik Pariwisata NHI Bandung" : "Contoh: Universitas Pendidikan Indonesia"}/>
              <p style={s.hint}>
                {form.status === "mahasiswa" ? "Nama perguruan tinggi tempat kamu kuliah saat ini" :
                 form.status === "fresh_graduate" || form.status === "alumni_nhi" ? "Nama perguruan tinggi tempat kamu lulus" :
                 "Isi dengan pendidikan terakhir kamu, contoh: SMA Negeri 1 Bandung atau Universitas Indonesia"}
              </p>
            </div>
            ...
            <div style={{ marginBottom: "1rem" }}>
              ...
              <input style={s.input} value={form.jurusan} onChange={e => upd("jurusan", e.target.value)}
                placeholder="Contoh: D4 Manajemen Perhotelan, S1 Pariwisata, D3 Tata Boga"/>
              <p style={s.hint}>Tulis lengkap jenjang dan nama program studi. Contoh: D4 Manajemen Perhotelan, S1 Ilmu Pariwisata, D3 Tata Boga</p>
            </div>

            <div style={s.row2}>
              <div>
                <label style={s.label}>Tahun Lulus {(form.status === "fresh_graduate" || form.status === "alumni_nhi") && <span style={{ color: "#ef4444" }}>*</span>}</label>
                ...
                <p style={s.hint}>
                  {form.status === "mahasiswa" ? "Kosongkan jika belum lulus" : "Tahun kelulusan dari program studi"}
                </p>
              </div>
              <div>
                <label style={s.label}>Bidang Minat / Keahlian <span style={{ color: "#ef4444" }}>*</span></label>
                ...
                <p style={s.hint}>Pilih bidang yang paling sesuai dengan minat dan keahlian utamamu</p>
              </div>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div style={s.card}>
            ...
            <p style={{ color: "#64748b", fontSize: "0.85rem", marginBottom: "1.5rem", lineHeight: 1.7 }}>
              Dokumen akan diverifikasi oleh panitia sebelum hari H. Pastikan foto dan dokumen jelas dan terbaca.
            </p>

            <div style={{ display: "grid", gap: "1.25rem" }}>
              <FileUpload
                label="Pas Foto Terbaru"
                hint="Format JPG/PNG · Maks 2MB · Foto formal dengan latar belakang polos (merah/biru). Wajah terlihat jelas, tidak menggunakan kacamata hitam."
                accept="image/*"
                file={form.foto}
              ...
              <FileUpload
                label="CV / Resume"
                hint="Format PDF · Maks 5MB · CV terbaru yang mencantumkan pengalaman, pendidikan, dan keahlian. Maksimal 2 halaman. Gunakan format yang rapi dan mudah dibaca."
                accept=".pdf"
                file={form.cv}
                onChange={f => upd("cv", f)}
                required
              />
              <FileUpload
                label="KTP atau Kartu Mahasiswa (KTM)"
                hint="Format JPG/PNG/PDF · Maks 2MB · Opsional. Scan atau foto KTP/KTM yang jelas. Mahasiswa aktif dapat menggunakan KTM."
                accept="image/*,.pdf"
                file={form.ktm}
                onChange={f => upd("ktm", f)}
              />
              <FileUpload
                label="Sertifikat Pendukung"
                hint="Format PDF/JPG · Maks 5MB · OPSIONAL. Sertifikat kompetensi, pelatihan, atau penghargaan yang relevan. Jika lebih dari satu, gabungkan menjadi satu file PDF."
                accept=".pdf,image/*"
                file={form.sertifikat}
                onChange={f => upd("sertifikat", f)}
              />
            </div>
          </div>
        )}
          ...
          <div>
            ...
            <div style={{ background: "rgba(20,184,166,0.05)", border: "1px solid rgba(20,184,166,0.2)", borderRadius: 12, padding: "1.25rem", marginBottom: "1.5rem" }}>
              ...
              <p style={{ fontSize: "0.82rem", color: "#64748b", lineHeight: 1.7 }}>
                Data pribadi Anda dilindungi sesuai <strong style={{ color: "#f1f5f9" }}>UU No. 27 Tahun 2022 tentang Pelindungan Data Pribadi (UU PDP)</strong>.
                Anda berhak mengakses, mengoreksi, dan menarik persetujuan atas penggunaan data Anda kapan saja.
              </p>
            </div>
              ...
              <label style={{ display: "flex", gap: "1rem", cursor: "pointer", alignItems: "flex-start" }}>
                ...
                <div>
                  <div style={{ fontWeight: 700, color: "#f1f5f9", marginBottom: "0.5rem", fontSize: "0.95rem" }}>
                    Saya setuju profil saya dapat dilihat oleh perusahaan yang berpartisipasi di GR2026 <span style={{ color: "#ef4444" }}>*</span>
                  </div>
                  <div style={{ fontSize: "0.82rem", color: "#64748b", lineHeight: 1.7 }}>
                    Dengan mencentang ini, data diri dan CV Anda dapat diakses oleh perusahaan/employer yang secara resmi terdaftar dan hadir di Grand Recruitment 2026.
                    Ini diperlukan agar employer dapat menghubungi Anda untuk proses rekrutmen selama dan setelah event berlangsung.
                  </div>
                </div>
              </label>
              {!form.consent1 && (
                <div style={{ marginTop: "0.75rem", padding: "0.75rem 1rem", background: "rgba(249,115,22,0.08)", border: "1px solid rgba(249,115,22,0.2)", borderRadius: 8, fontSize: "0.8rem", color: "#fdba74", lineHeight: 1.6 }}>
                  ⚠️ Tanpa consent Layer 1, panitia tidak dapat membagikan data Anda ke employer. Anda tetap bisa hadir namun harus membawa CV hardcopy dan menyerahkan langsung ke booth employer yang Anda datangi.
                </div>
              )}
            </div>
            ...
            <div style={s.card}>
              ...
              <label style={{ display: "flex", gap: "1rem", cursor: "pointer", alignItems: "flex-start" }}>
                ...
                <div>
                  <div style={{ fontWeight: 700, color: "#f1f5f9", marginBottom: "0.5rem", fontSize: "0.95rem" }}>
                    Saya setuju profil saya dapat dibagikan ke employer yang tidak hadir secara fisik (employer online)
                  </div>
                  <div style={{ fontSize: "0.82rem", color: "#64748b", lineHeight: 1.7 }}>
                    Dengan mencentang ini, data dan CV Anda juga dapat diakses oleh perusahaan yang berpartisipasi secara virtual/online di GR2026.
                    Ini membuka peluang lebih luas dari perusahaan yang tidak hadir secara fisik namun tetap ingin merekrut.
                  </div>
                </div>
              </label>
              <div style={{ marginTop: "1rem", padding: "0.75rem 1rem", background: "rgba(129,140,248,0.08)", border: "1px solid rgba(129,140,248,0.2)", borderRadius: 8, fontSize: "0.8rem", color: "#a5b4fc" }}>
                🚧 Fitur Employer Online sedang dalam pengembangan. Consent ini akan aktif saat fitur diluncurkan.
              </div>
            </div>

            {/* Rights info */}
            <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 12, padding: "1.25rem" }}>
              <div style={{ fontWeight: 700, color: "#94a3b8", marginBottom: "0.75rem", fontSize: "0.85rem" }}>Hak-hak Anda sebagai Subjek Data:</div>
              {[
                "✓ Hak mengakses data pribadi yang kami simpan",
                "✓ Hak mengoreksi data yang tidak akurat",
                "✓ Hak menarik persetujuan kapan saja melalui portal jobseeker",
        ...
        {step === 4 && (
          <div style={s.gold}>
            <div style={s.secHd}>📋 Ringkasan Pendaftaran</div>
            <p style={{ color: "#64748b", fontSize: "0.85rem", marginBottom: "1.5rem" }}>Periksa kembali data Anda sebelum menyelesaikan pendaftaran.</p>

            {[
              { label: "Nama Lengkap", val: form.namaLengkap },
              { label: "Nomor Identitas", val: `(${form.idType === "sim" ? "SIM" : form.idType === "passport" ? "Passport" : "KTP"}) ${form.nik}` },
            ...
            <div style={{ marginTop: "1rem", padding: "1rem", background: "rgba(255,255,255,0.02)", borderRadius: 10 }}>
              ...
              <div style={{ fontSize: "0.82rem", color: "#64748b" }}>
                {form.consent1 ? "✅" : "❌"} Employer offline GR2026 &nbsp;·&nbsp;
                {form.consent2 ? "✅" : "⬜"} Employer online
              </div>
            </div>
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: "1rem", marginTop: "1rem" }}>
          ...
          {step < STEPS.length - 1 ? (
            <button style={{ ...s.btnPri, flex: 2, opacity: canNext() ? 1 : 0.4, cursor: canNext() ? "pointer" : "not-allowed" }}
              onClick={() => canNext() && setStep(s => s + 1)}>
              Lanjut →
            </button>
          ) : (
            <button style={{ ...s.btnPri, flex: 2, opacity: (canNext() && !uploading) ? 1 : 0.4, cursor: (canNext() && !uploading) ? "pointer" : "not-allowed" }}
              onClick={() => canNext() && !uploading && handleSubmit()}>
              {uploading ? "⏳ Mengupload..." : "Selesaikan Pendaftaran 🎉"}
            </button>
          )}
        </div>
      </div>
